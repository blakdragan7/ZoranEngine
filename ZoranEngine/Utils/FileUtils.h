...
ZoranEngine_EXPORT bool MoveToChar(std::fstream& inFile, char theChar);
...
ZoranEngine_EXPORT bool GetLine(std::fstream& inFile, std::string& line, std::string delims);
// Same as GetLine except it searches for the entire string for each string in delims and returns when one is found
// returns true if found a new line with delim or false if it hasn't
ZoranEngine_EXPORT bool GetLineVar(std::fstream& inFile, std::string& line, std::vector<std::string> delims);
...
ZoranEngine_EXPORT int GetFilesInDir(std::string inDir, std::string fileMask,bool recurse, std::function<void(std::string, std::string)> callback);